...
- [x] **CAT-UI-006**: When the user confirms deletion of a SubCategory, the system shall delete the SubCategory and all its associated events. When the user confirms deletion of a MetaCategory, the system shall, within a single database transaction, promote each SubCategory to a MetaCategory (resolving any null fields to the parent's current values at the time of deletion) and delete the MetaCategory and all its own events; the promoted SubCategories and all their events are preserved.
