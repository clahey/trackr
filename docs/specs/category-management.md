...
- [x] **CAT-UI-055**: While editing a SubCategory, the emoji field shall display an "Inherit" toggle to the left of the text field; when on, the field is non-editable and shows the parent's emoji; when off, the field is editable and shows the stored custom value; tapping anywhere in the toggle row (toggle or label) shall flip the state.
