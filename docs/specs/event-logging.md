...
- [ ] **EL-UI-078**: When seeding a new event value for a category, the system shall: if `category.resolvedDefaultValue` is non-null and `matchesValueType(resolvedDefaultValue, resolvedValueType)` is true, use `resolvedDefaultValue.toValueUIState()`; otherwise use `defaultValueUIStateForType(resolvedValueType)`.
