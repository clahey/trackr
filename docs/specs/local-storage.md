...
- [x] **LS-BE-015**: `saveCategory` shall run the category upsert and every optional step it was given — the event migration, the destination-group reindex, and the reminder upsert — inside one Room `@Transaction`, and when writing a reminder shall read the stored row's `nextFireAt` and write it back within that same transaction. A null `reminder` argument shall leave the stored reminder row untouched, and the method shall offer no way to delete one. The category upsert shall precede the reminder upsert, since `ReminderEntity`'s foreign key (LS-BE-072) requires the category row to exist. This is the storage mechanism behind the atomicity that several specs require of it — `docs/specs/reminders.md` REM-DATA-006 and REM-DATA-008 for the reminder write, `docs/specs/category-management.md` CAT-UI-080 and CAT-UI-083 for the reparent-and-reindex; those specs own *what* must hold, this one owns *how* it is achieved here.
...
- [x] **LS-BE-073**: Every `ReminderEntity` column carrying a mode's configuration — `times`, `windowStart`, `windowEnd`, `occurrencesPerDay` — shall be declared `NOT NULL`, so a row always holds both modes' settings whichever mode is active; this is the storage mechanism behind the preservation rule (`docs/specs/reminders.md`, REM-DATA-002). `nextFireAt` shall remain nullable, null meaning not armed. A migration that makes one of these columns `NOT NULL` shall convert any stored null to the value the decoder already read it as, rather than dropping the row.
