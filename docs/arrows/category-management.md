...
9. **Both ViewModels moved onto the merged repository `saveCategory` (2026-08-25).** `local-storage` collapsed five category-write methods into one with optional arguments (`docs/arrows/local-storage.md` finding 10), and this segment held all ten call sites. `CategoryListViewModel` had four places branching on `orderedSiblingIds != null` to pick between a `move*` and a `save*` variant — that branching is now argument passing, since a null `orderedSiblingIds` simply skips the reindex. `CategoryEditViewModel.save()`'s two-branch call became one, with the value-type comparison it was already doing feeding a `migrateEvents` Boolean instead of selecting an overload. No spec change: CAT-UI-080/081/083 describe behavior without naming repository methods, which is why they needed nothing. No behavior change either — the eight list-screen call sites never touched reminders before and still don't, now by leaving an argument off rather than by calling a different function.
