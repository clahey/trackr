...
**Inline category creation.** The top-level grid always ends with a persistent full-width **"+ New category"** tile (spanning the grid, icon + label in a row); the drill-down grid always ends with a full-width **"+ New subcategory"** tile (creating a child of the expanded MetaCategory). Category tiles auto-shrink their name label to one line down to a floor, then ellipsize (EL-UI-095), so a long name like "Medication" stays readable in the narrow 3-column grid; the "+ New …" tiles are full-width precisely so their longer labels never need shrinking. The shrink-to-fit itself (`AutoShrinkLabel` in `ui/home/QuickLogSheet.kt`) is a hand-rolled measure-and-step loop because the pinned Compose BOM (2025.01.01) predates `BasicText`'s native `autoSize` param, added in BOM 2025.04.01 — worth revisiting as a deletion candidate if the BOM is ever bumped past that point. On a fresh install with no categories, step 1 shows the "Choose a category" heading over a grid holding only the "+ New category" tile — the primary action (FAB → step 1) is never a dead end (EL-UI-090, EL-UI-091). Tapping "+ New category" navigates to Category Edit for a new top-level category; "+ New subcategory" navigates with `parentId` set to the expanded MetaCategory. Both first set `QuickLogViewModel.pendingCategoryCreate` so the sheet reopens on return; navigating away dismisses the sheet, but `QuickLogViewModel` survives on the back stack, preserving `expandedMetaCategoryId` (EL-NAV-020).
...
A shared composable (`ui/components/TimePickerDialog.kt`) wrapping M3's `TimePicker` in an `AlertDialog`. Material3 ships a `DatePickerDialog` but no `TimePicker` equivalent, so without this every caller hand-rolls the same scaffolding. Takes `initial: LocalTime`, `onConfirm: (LocalTime) -> Unit`, and `onDismiss: () -> Unit`: the picker opens on `initial`, Confirm reports the selected time, and Cancel or a tap outside reports a dismissal.

Closing the dialog is the caller's job rather than the dialog's. Every caller already owns the state that decides whether it is showing, and most of them do other work in the same handler — a self-closing dialog would leave that state to be cleared anyway.

Used by `TimestampField` above, and by the reminder editor's time chips and Start/End window fields (`docs/llds/reminders.md § Category Edit Integration (UI)`).
...
| Date/time editing in `TimestampField` | Independent — each sub-field opens its own picker and commits only its own component | Confirming the date chains straight into a time picker | Most edits correct one component, not both. Chaining makes every date fix a two-dialog trip, and the time picker it opens has no good starting value — the existing time is what the user just declined to change, and midnight is arbitrary | Single seconds field; HH:MM:SS text entry | Three fields allow independent editing of each component; single seconds field is unintuitive for durations > 60s |
