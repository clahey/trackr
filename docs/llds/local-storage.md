...
All reads return `Flow`. Writes are `suspend`. `saveCategory` and `saveEvent` are upserts. `getEvents` accepts optional epoch-millis bounds; null = unbounded. The repository dispatches each bound combination to its own `EventDao` query: `getAll`, `getAllFrom`, `getAllBefore`, `getAllInRange`.
...
**`onStartup`:** called once per app process, launched fire-and-forget when the first Activity is created (`docs/llds/app-shell.md § Startup Sequence`) — it does not block the first UI frame (per LS-BE-041). A process Android creates only to deliver a reminder alarm never calls it; which lifecycle event triggers the hook is app-shell's decision, not this seam's. `LocalTrackrRepository` uses it to scan `filesDir/images` and delete any file not referenced by a DB event row, a purely additive cleanup with no ordering requirement against the UI. Future implementations may use it for different initialization behavior (sync, token refresh, etc.) — if that behavior is something the UI depends on, the fire-and-forget launch must be revisited to actually block first frame.
...
| Scope of `onStartup()` | Persistence-only; other segments' startup work gets its own call | Fold every segment's startup work into this one hook | Re-arming reminder alarms needs `AlarmManager`, which sits outside this segment's persistence seam. Separate hooks also let each be triggered at the lifecycle point it wants — the image scan on first Activity, the reminder reconcile on process start |
