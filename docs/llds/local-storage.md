...
| Scope of `onStartup()` | Persistence-only; other segments' startup work gets its own call from `TrackrApplication.onCreate()` | Fold every segment's startup work into this one hook | Re-arming reminder alarms needs `AlarmManager`, which sits outside this segment's persistence seam |
