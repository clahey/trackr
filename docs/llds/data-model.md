...
| Storing an inherited value | A nullable column on the subcategory row, where null means inherit | Denormalize at write time (copy the parent's values onto the child); a separate override-flag column beside each inheritable field | A copy taken at write time is wrong the moment the parent is edited, and correcting it means rewriting every child. An override flag carries information the nullable column already holds, at the price of a second column per inheritable field. One nullable column both stores the value and answers whether it was set |
