...
1. **Keystore execution** (structure decided — see Decisions; this is the remaining work). On the critical path to a signed release build (step 9):
   - End state: two per-app aliases in `~/keystores/upload-keystore.jks`. The file originally held one generically-named `upload` alias (a `PrivateKeyEntry` created 2024-11-15). Rename it to `golf-score` (`keytool -changealias`, a local relabel — the alias name isn't in the cert, so Play is unaffected), then generate a new `trackr` alias alongside it (`keytool -genkeypair`, RSA 2048, `-validity 9125`). Needs the keystore password; this is a PKCS12 file, so key passwords equal the store password.
   - Key validity **verified** (2026-07-03): the existing key (now `golf-score`) is valid Nov 15 2024 → Nov 09 2049 — a 25-year Studio-wizard key, well past the ~2033 checkpoint. The new `trackr` alias is generated with the same 25-year window. Re-confirm with a read-only `keytool -list -v`.
   - ✅ **Resolved** — signing is handled by Android Studio's Generate Signed Bundle / APK wizard (see Decisions row "Release-signing password handling"). `app/build.gradle.kts` deliberately has **no** `release` `signingConfig` — a comment there records why — so it neither repeats golf-score's `signingConfigs.debug` mistake nor tries (and fails) to prompt for a password from Gradle. CLI `bundleRelease` produces an unsigned bundle.
