...
| Reminder | Full field design (mode, times/window, active days, notification content toggle, permission handling) owned by `docs/llds/reminders.md § Category Edit Integration` | Always | N/A — reminders are independent per category, not inherited |
...
- **Initial load gating (CAT-UI-018):** the init block issues one coroutine per initial-state read, each flipping its own flag on *completion* — not on success, so a category with no reminder row (the common case) still opens the gate. `isLoaded: StateFlow<Boolean>` is the conjunction of the flags relevant to the current mode: edit mode, the category read and the reminder read; SubCategory create, the parent read; MetaCategory create, the color-index read. Until it is true the screen disables every input and the Save button, and `save()` returns without persisting. This is what makes the seeding writes safe: the init block writes form state directly, so without the gate a read landing after the user had already typed would silently overwrite their input, and a save issued before a read landed would persist defaults over stored values — writing a `MetaCategory` over a `SubCategory` whose parent hadn't loaded, or a disabled default reminder over a configured one. The gate does not delay the stale-category guard above: a null category navigates back immediately, without waiting for the reminder read.
...
- **Dirty tracking:** two flags, both flipped by `markEdited()` in every field setter (the init block writes fields directly, no dirty side-effect). `isDirty: StateFlow<Boolean>` starts `true` in create mode / `false` in edit mode and drives **Save-button visibility** (CAT-UI-067: create shows Save immediately). `hasUserEdits: StateFlow<Boolean>` starts `false` in both modes and drives the **back-guard** (CAT-NAV-006): navigating back (hardware back or nav icon) shows the UnsavedChangesDialog (Save / Discard / Cancel) only when `hasUserEdits` is true — so an immediate back from a freshly-opened create screen does **not** warn, even though its Save button is already showing. Save calls `save()` which navigates back via `SaveResult.Success`; Discard navigates back without persisting; Cancel dismisses the dialog. `UnsavedChangesDialog` is a shared composable in `ui/components/`.
...
- **Reporting the created id.** `save()` sets `savedCategoryId: StateFlow<String?>` to the persisted category's id (create or edit) alongside `SaveResult.Success`. On save success the screen calls a dedicated `onCategoryCreated(id)` callback **only when `!isEditMode`** (a genuinely new category) — the navigation layer attaches the id to the initiating screen's back stack entry (`SavedStateHandle["created_category_id"]`) *before* `onNavigateBack` pops, so the caller (e.g. the quick-log flow, CAT-NAV-020) can react. Delete also emits `SaveResult.Success` but never sets `savedCategoryId` and is gated out by `!isEditMode`, so it reports no id; a create originating from the category list writes the key on the list's entry, which the list ignores — consumers opt in. `SaveResult.Success` stays a bare object (shared app-wide); the id rides the separate `savedCategoryId` flow rather than being folded into it.
- **Deletion counts are read at request time, not held as state (CAT-UI-019).** `requestDelete()` reads `getEventCountForCategory(id, includeSubCategoriesWithNullType = false)` and `getSubCategoryCount(id)` inside its own coroutine and passes the results straight to `deletionConfirmationIfNeeded`. Holding them as `stateIn(..., Eagerly, 0)` flows instead would seed both at 0 — the exact shape that satisfies CAT-UI-004's silent-delete condition — so a tap landing before the queries emit would delete a populated category with no confirmation. Reading at the point of decision removes the placeholder rather than gating around it, and nothing else observes these counts: the dialog renders from `DeleteConfirmation`'s own fields. `CategoryListViewModel` reads them the same way for the same decision.
- **`performDelete(id)`** is the shared tail of both delete paths: `repository.deleteCategory(id)`, then `reminderScheduler.cancel(id)` (CAT-UI-007's ordering), then clear any pending confirmation and emit `SaveResult.Success`. `requestDelete()` (no confirmation needed) and `confirmDelete()` (user confirmed) both end here, so the cancel-the-alarm step cannot be added to one path and forgotten on the other.
...
- `save()`: persists via `repository.saveCategory(category, reminder, migrateEvents)` — one transaction covering the category upsert, the reminder write, and, when `migrateEvents` is set, the event migration (including inheriting SubCategory events). `migrateEvents` is true only in edit mode when the effective `valueType` actually changed. This screen is the only caller that passes a reminder; the list screen's reparent paths leave the argument off, which leaves the stored reminder untouched (see § Value Type Migration)
...
`TrackrRepository.saveCategory(category, migrateEvents = true)` runs the category upsert and all event value updates inside a single Room transaction, ensuring the database is never left in a partially-migrated state. The events are converted to the category's own `resolvedValueType`, so the call needs to know only *that* a migration is wanted, not what the type was before.
...
- **Calls the widget's `onSettled`.** Every path through `onMove` — the plain persist, the migrating persist, the dialog's Cancel, *and a rejected persist* (see Rejection below) — ends by calling `onSettled`, exactly once. For the plain/migrating persist, that's at the end of the same coroutine (after `repository.saveCategory` returns *or throws*). For the dialog path, `onSettled` can't be called from `onMove`'s own coroutine, since persistence (or abandoning it) doesn't happen until the user acts on a separate dialog later — it's threaded into `PendingValueTypeConfirmation` instead and invoked from both `confirmPendingValueTypeChange()` (after the migrating persist completes or throws) and `cancelPendingValueTypeChange()` (immediately, nothing persisted). The widget doesn't know or care which of these happened; it just unfreezes and re-reads `categories`, which by then either reflects the move or doesn't. See `docs/llds/drag-reorder-list.md § Settling` for the widget side of this protocol.

**Persistence.** A drop persists through the repository's general reparent-and-reorder capability — `saveCategory(category, orderedSiblingIds = ..., migrateEvents = ...)`, with `migrateEvents` set when the effective value type changed (see the Value-type-change check above) — defined and detailed in `docs/llds/local-storage.md § LocalTrackrRepository`: one transaction that upserts the reparented row under the DM-DATA-028 childlessness guard, then re-reads the destination group's live members and dense-reindexes them by reconciling `orderedSiblingIds` against those members per CAT-UI-083. That re-read/reconcile mechanism is not drag-specific — it protects any reparent-with-explicit-order caller — so it lives in the data-domain LLD; this segment only supplies the two inputs and owns where they come from:

- `category`: the moved item, already reconstructed as the correct domain variant (`MetaCategory` if `newParentId == null`, else `SubCategory` with the resolved `parent`) by the adapter.
- `orderedSiblingIds`: the **destination** group's id order as the *drop* saw it — all `MetaCategory` ids for a top-level move, or the target parent's children ids (including the moved item) for a nest; mirrors `DragMoveResult.orderedSiblingIds`. It is read from the widget's rendered `items` (a `getCategories()` Flow emission) **outside any transaction**, and — for a drop routed through a CAT-UI-081 confirmation dialog — held across an arbitrarily long user pause before it is applied. This staleness is *why* the repository reconciles it against live members rather than trusting it as a membership list; the dialog widening the window is drag-specific, but the reconciliation it necessitates is the general capability above.

The moved category's target parent being deleted concurrently is **not** handled — the upsert leaves the row pointing at a missing parent, which `toDomainList()`'s orphan recovery surfaces as a top-level category (see Open Question 9).

**Rejection: a leaf that gained children since the snapshot.** The adapter marks a childless `MetaCategory` `canBecomeChild = true` from the *snapshot*, so the widget offers nesting it under another group. Between that snapshot and the drop actually persisting (widened, again, by a possible CAT-UI-081 dialog), the category can concurrently gain a `SubCategory` — via "Create subcategory" or another "Add to group" — which would make the requested nest a two-level-deep violation. This invariant is enforced *authoritatively inside the transaction*: `saveCategory`'s existing `requireNoChildren` guard re-reads the child count within the write and throws `CategoryHasChildrenException` when the reconstructed `SubCategory` still has children, rolling the whole transaction back so no illegal nest is ever persisted. The ViewModel does **not** re-check the child count itself before persisting — that would reintroduce the same out-of-transaction TOCTOU the reconciliation above closes; the in-transaction guard is the single source of truth. `persistReparent` therefore treats persistence as fallible: it catches that rejection and, on every reparent entry point that shares it, (a) calls `onSettled` exactly once anyway so the drag widget unfreezes and animates the row back to its origin (the same no-op bounce-back as a cancelled dialog — the persist simply didn't change `categories`), and (b) surfaces a snackbar on the category list reading "'<name>' now has subcategories and can't be nested." All three reparent entry points route through `persistReparent`/`persistOrReject` and share this coverage. "Move to another group" (CAT-UI-052) is **not** immune despite operating on what the menu snapshot saw as a `SubCategory`: between that snapshot and the commit the moved item can be concurrently *promoted* to a `MetaCategory` (via "Remove from group") and gain children — the same background-sync-gated concurrency family as Open Question 10 — whereupon `reparentCategoryInternal` re-reads the live category and reconstructs it back as a `SubCategory` while its child rows survive, so the same in-transaction guard rejects it and `persistOrReject` catches it identically. That race is unreachable on today's single-user local app (promoting the item means leaving the open picker), but the safety comes from the guard, not from type immunity — so drag (CAT-UI-080), "Add to group" (CAT-UI-051), and "Move to another group" (CAT-UI-052) are all covered. The rejection is distinguished by a dedicated exception type: `requireNoChildren` throws `CategoryHasChildrenException` (a subclass of `IllegalArgumentException`, preserving the DM-DATA-028 guard's existing semantics), which `persistReparent` catches *precisely* — so it surfaces the snackbar only for this specific rejection, never swallowing an unrelated argument error, and stays correct even if another `require` is later added to these write paths.
...
- `getEventCountForCategory(id, includeSubCategoriesWithNullType = false)` — own events only; read at delete-request time to size the confirmation (CAT-UI-019), and for SubCategory event counts.
...
- `getSubCategoryCount(id)` — count of SubCategories with this parentId; read at delete-request time alongside the event count to decide whether a MetaCategory's deletion needs confirmation (CAT-UI-019).

All three return `Flow<Int>` so the UI stays current if events or subcategories change while the edit screen is open.

## Starter Categories

A fixed starter set lets a fresh install go from empty to logging in one tap (surfaced by the timeline empty state EL-UI-092 and the About screen APP-UI-010). The set: Mood (Scale), Sleep (Duration), Water (Number, unit "glasses"), Exercise (Exercise), Medication (Number, unit "mg"), Pain (Scale).

- **Seed definition** lives in the UI layer (`ui/StarterCategoryDefaults.kt`) as `getStarterCategoryInputs(context)`, a plain (non-`@Composable`) function that builds the `StarterCategoryInput` list directly, resolving each name (and, for Number types, unit) from a `@StringRes` via `Context.getString` — names/units are resources so they localize (HLD localization decision), everything else (color, emoji, value type) is a plain constant. It's a plain function rather than a `@Composable`/`remember`-backed one so resolution happens once, at the point of use (the "Add starter categories" click), not on every composition of a screen that offers it.
- **Creation** goes through the repository as **ordinary top-level `MetaCategory`s** — no "starter" flag or special type; once created they are indistinguishable from hand-made categories (reorder, edit, delete). The write is a single transactional `addStarterCategories` (LS-BE-093) rather than a loop of `saveCategory`: looping would assign each `sortOrder = min − 1` (reversing the set) and draw colors off the `nextCategoryColorIndex` counter. The transaction instead pins explicit palette colors and a top-of-list `sortOrder` block in the listed order.
- **Idempotency** is "add only what's missing": it creates only starters whose name is not already present (case-insensitive, current-locale name), so it's safe to invoke repeatedly from the About screen. Matching on the resolved name has a minor limitation — a device-language change between runs could re-add a starter under the new locale's name; accepted for v1 over adding a hidden stable-key column. The insert-selection + ordering is the pure `starterCategoriesToInsert` (`domain/StarterCategories.kt`), shared with `FakeTrackrRepository` and unit-tested in isolation.
...
Category Edit
    ...
    ├── [save]                   → back to caller  (button only visible when dirty)
    │        └── new category: report id via SavedStateHandle["created_category_id"] before popping (CAT-NAV-020)
...
| Initial-state reads not transactional | Independent per-read coroutines, each gating on its own completion (CAT-UI-018); no combined read | One combined read — `combine` of both flows, or a Room relation query — inside a single transaction | These reads seed form state the user is about to overwrite, so a torn seed is stale rather than corrupting. The one durable torn-read risk, `ReminderScheduler` writing `nextFireAt` between the two reads, is invisible here (`ReminderUIState.fromStored` ignores `nextFireAt`) and is already handled inside `saveCategory`'s own transaction. Independent flags also keep each read separately observable in tests |
| Rendering the load gate | `enabled = isLoaded` threaded through every field and each private composable that hosts one; the two raw `clickable` surfaces (color swatches, emoji quick-picks) and `OutlinedFieldBox` dim by hand to Material's disabled tokens | A single wrapper on the form column — `alpha` plus a `pointerInput` loop consuming events in the `Initial` pass | The wrapper does not block accessibility: TalkBack activates through the semantics tree, not pointer events, so every field stayed reachable and announced as editable. `enabled = false` drops the click semantics at the source. It also buys Material's own disabled styling per widget; the handful of surfaces Material doesn't style get `DISABLED_ALPHA` explicitly so the form dims to one level. Cost is that a field added later must opt in — the wrapper would have covered it automatically |
| Save button while loading | Rendered, disabled | Hidden until loaded; rendered enabled and inert | CAT-UI-067 deliberately shows Save immediately in create mode, so hiding it contradicts an existing decision; an enabled button that silently does nothing is worse than a visibly disabled one |
| Reporting a newly-created category's id | Separate `savedCategoryId` flow + `onCategoryCreated(id)` callback, gated on `!isEditMode`; navigation layer writes `SavedStateHandle["created_category_id"]` on the caller's entry | Overload `SaveResult.Success` to carry the id; have the caller diff the category list on return to find the new one | `SaveResult.Success` is shared across event/category editors — overloading it ripples widely; list-diffing races with the Room `Flow` and mis-fires on concurrent edits. A dedicated flow keeps the producer low-coupling and lets consumers (quick-log; the category list, which ignores it) opt in — CAT-NAV-020 |
...
8. **Typed rejection exception for reparent (relates to CAT-UI-084)** — *Resolved and implemented.* `requireNoChildren` throws a dedicated `CategoryHasChildrenException` (subclass of `IllegalArgumentException`, so DM-DATA-028's guard semantics are unchanged) and `persistReparent` catches exactly that type, so the reparent-rejection snackbar can never be triggered by an unrelated argument error and the catch stays precise regardless of any future guards added to the move/save write paths. See § Drag-to-Reorder rejection handling above.
9. **A drop whose target group was deleted concurrently silently becomes a top-level move.** In `onDragMove`, `newParent` is resolved by `getCategoryById(newParentId) as? MetaCategory`; if the target group was deleted between the snapshot and the drop, this is `null`, and `reconstructForMove` then promotes the dragged item to a top-level `MetaCategory` instead of nesting it — the drop lands, just not where aimed. This is the reparent-target-deleted analogue of the `sortOrder` orphan-recovery escape hatch and is currently unspecified/untested. **Preferred eventual resolution: reject and bounce back**, reusing the CAT-UI-084 mechanism (`persistOrReject` → `onSettled` fires so the row animates back to origin, plus a snackbar such as "'<group>' no longer exists.") — leaving the item where it was is less jarring than silently promoting a SubCategory out of its group and re-landing it at the bottom of the top-level list. **Reachability gate:** on a single-user local app the target can't be deleted mid-drag, so this is unreachable today; **background sync** is what makes a concurrent delete real, and is the trigger to actually build the rejection. Deferred until then; the current fallback is acceptable interim precisely because it degrades to valid data and — unlike the pre-CAT-UI-084 childlessness path — cannot freeze or crash the app.
10. **A type-change confirmation dialog can be invalidated by a concurrent write while it is open.** When an inheriting `SubCategory` is dragged to a parent whose type differs (CAT-UI-081), the pending confirmation captures a *snapshot* of the reconstructed (still-childless) SubCategory. If, while the dialog is open, that category is concurrently promoted to a `MetaCategory` and gains children, confirming the dialog persists the stale snapshot and the in-transaction childlessness guard (DM-DATA-028) re-reads the now-present children and rejects it with `CategoryHasChildrenException`. This is the confirm-path (deferred-persist) analogue of CAT-UI-084's drag rejection and of Open Question 9's vanished-target — **same concurrency family, same reachability gate**: a single-user local app can't mutate a category behind a modal dialog, so it is unreachable today and becomes real only under **background sync**. Current behavior degrades gracefully **and is verified** (`CategoryListViewModelDragTest`): `confirmPendingValueTypeChange` routes through `persistOrReject`, so on rejection the row still settles — `onSettled` fires exactly once, the drag widget never freezes — the dialog clears, and the "'<name>' now has subcategories and can't be nested" snackbar shows; nothing is persisted. **Not yet handled:** the dialog does not *reactively* invalidate — between the concurrent change and the user acting, it sits stale, and the user only discovers the invalidation on confirm. Preferred eventual resolution (deferred until background sync, alongside Open Question 9): watch the pending confirmation's subject and, if invalidated, dismiss the dialog and settle rather than waiting for a confirm that can only fail. Acceptable interim precisely because — like the other members of this family — it cannot freeze or crash the app.
11. **A category deleted between the edit screen's two initial reads leaves a live editor for a row that no longer exists.** The category read succeeds, the delete commits (cascading the reminder row away), and the reminder read returns null — which CAT-UI-018 counts as a completed load, so the gate opens and the stale-category guard never fires, because the category *was* found. Saving then re-creates the category without its events. Accepted rather than fixed: the reads are one-shot `.first()` calls, so nothing on this screen observes the category after load at all — a delete landing a millisecond *after* the gate opens produces the same outcome, and closing the window properly means moving the whole screen to observed flows, a change well beyond the load gate. **Same reachability gate as Open Questions 9 and 10**: delete is initiated either from this screen or from the list screen it covers, so a single-user local app can't reach it; background sync is what makes it real.
...
- `docs/llds/reminders.md` — the Reminder section's full design (owns the Category Edit screen field referenced above) and the delete-flow alarm-cancel call referenced above
