...
Because children are promoted (not deleted), the event count shown in the delete confirmation covers only the MetaCategory's **own** events, not its children's. The dialog also states the number of SubCategories that will be promoted. Immediate silent deletion (no dialog) is allowed only when the MetaCategory has zero own events **and** zero SubCategories; if either is non-zero, show a confirmation dialog.
...
| Emoji | Single-character text field (system emoji keyboard) | Always | Toggle: "Use [ParentName]'s emoji / Custom". When inheriting, field shows parent's emoji subdued and non-editable |
...
- **Per-field form state for inheritable fields** (SubCategory mode only): `emojiState: MutableStateFlow<String?>`, `colorState: MutableStateFlow<Long?>`, `valueTypeState: MutableStateFlow<ValueType?>` — null = inherit. For MetaCategory mode, these are always non-null.
...
- `ownEventCount: StateFlow<Int>` — live count of events belonging directly to this category, from `repository.getEventCountForCategory(categoryId, includeSubCategories = false)`; used to determine whether a confirmation dialog is needed before deletion (for a MetaCategory, also gated on `subCategoryCount`)
