...
- **Seed definition** lives in the UI layer (`ui/StarterCategoryDefaults.kt`) as `StarterCategorySpec`s pairing a `@StringRes` name (and, for Number types, a `@StringRes` unit) with a preset palette color, emoji, and value type — names/units are resources so they localize (HLD localization decision). `rememberStarterCategoryInputs()` resolves them (via `Context.getString`, usable inside `map` unlike `stringResource`) into resource-free `StarterCategoryInput`s for the data layer.
