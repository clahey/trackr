...
- `save()`: persists via `repository.saveCategoryWithReminder(category, reminder, migrateFromType)` on both paths — one transaction covering the category upsert, the reminder write, and, when `migrateFromType` is non-null, the event migration (including inheriting SubCategory events). `migrateFromType` is `originalValueType` only in edit mode when the effective `valueType` actually changed; otherwise null and no migration runs. The list screen's reparent paths use `saveCategory`/`saveCategoryAndMigrateEvents` instead — they have no reminder to write (see § Value Type Migration)
