...
- **Room Database** — three tables: `categories`, `events`, `reminders`. Categories form a two-level hierarchy of top-level categories and their subcategories, where a subcategory inherits any field it does not set itself.
...
- **Reminder Scheduler** — computes and arms per-category reminder alarms via `AlarmManager`, one per category with reminders enabled, and posts the notification when one fires (tapping it deep-links into quick-log for that category). Reminder configuration is edited from the category edit screen, owned by category management; the scheduler itself is a standalone component the rest of the app does not otherwise depend on. Reminders that have fired and not yet been dealt with are listed at the top of the timeline, read from the notification shade rather than stored.
...
**Orphan recovery when the UI starts.** Because DB row deletion and image file deletion are separate operations, a process kill between them can leave orphaned files. `onStartup()` scans `filesDir/images` and deletes any file not referenced by a current event row in the database, recovering storage without data loss. It runs once per process, triggered by the first Activity rather than by `Application.onCreate`. Only UI activity produces orphans, so the uncollected set cannot grow while the app is merely being woken to deliver alarms — deferring the scan to the next UI start runs it at least as often as orphans are produced, and keeps a full event-table read off a wake that exists to post a notification. Stale category references (events whose category was deleted mid-session) surface gracefully as orphaned events with a missing header rather than crashing.

*Alternatives considered:* strict transactional cleanup across DB + filesystem (impossible without two-phase commit); ignore orphans (accumulates wasted device storage); scan on every process start (pays a whole-table read on every alarm wake to collect a set that wake cannot have added to).
